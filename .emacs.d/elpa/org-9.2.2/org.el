...
ust have a smaller ASCII number than `org-lowest-priority'."
  :group 'org-priorities
  :type 'character)

(defcustom org-lowest-priority ?C
  "The lowest priority of TODO items.  A character like ?A, ?B etc.
ust have a larger ASCII number than `org-highest-priority'."
  :group 'org-priorities
  :type 'character)
...
ake sure that you only list packages here which:
...
atch group 3 is set to the value when it exists.  If there is no
...
SG is a string to replace \"Refile\" in the default prompt with
...
enus are added if keys require more than one keystroke.  Tabs
...
ATCHER is a function accepting three arguments, returning
...
ATCH is a string with match syntax.  It can contain a selection
...
ATCH is a tags/property/todo match as it is used in the agenda tags view.
...
odifications are made by side-effect.  Return new alist."
...
atch data is set according to this regexp when the function
returns.
...
ove to the next element at the same level, when possible."
  (interactive)
...
ove to the previous element at the same level, when possible."
  (interactive)
