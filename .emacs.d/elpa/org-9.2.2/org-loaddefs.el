...
ove the current subtree to the archive.
...
ove the cursor to the Nth column in the current table line.
...
ove column to the right.

\(fn)" t nil)

(autoload 'org-table-move-column-left "org-table" "\
ove column to the left.

\(fn)" t nil)

(autoload 'org-table-move-column "org-table" "\
ove the current column to the right.  With arg LEFT, move to the left.
...
ove the current table line down.  With arg UP, move it up.
